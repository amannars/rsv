...
reg_print <- function(glm){
  glm_summary <- summary(glm)
  coeff <- glm_summary$coefficients[,1]
  CI <- confint(glm)
  p_value <- glm_summary$coefficients[,4]
  table_out <- cbind(coeff, CI[,1:2],p_value)
  stars <- rep(" ",nrow(CI))
  stars[p_value <= 0.10] <- "*"
  stars[p_value <= 0.05] <- "**"
  table_out <- cbind(coeff, CI[,1:2],p_value, stars)
  print(data.frame(table_out))
}
...
data <- merge(CFQ,covariates)
print(paste("Missing CFQ data", nrow(covariates)-nrow(CFQ)))
print(paste("Final Participant Count",nrow(CFQ)))
...
print(paste("Response:", CFQ_names[2]))
svy_glm <- svyglm(CFDCSR~RIAGENDR+DMDMARTL+RIDRETH1+DMDEDUC2+Age_cat+BMI_cat+FIPR_cat+SMQ_cat+ALQ_cat+DMDBORN4,
                  design=svy_design)
reg_print(svy_glm)

print(paste("Response:", CFQ_names[3]))
svy_glm <- svyglm(CFDCIR~RIAGENDR+DMDMARTL+RIDRETH1+DMDEDUC2+Age_cat+BMI_cat+FIPR_cat+SMQ_cat+ALQ_cat+DMDBORN4,
                  design=svy_design)
reg_print(svy_glm)

print(paste("Response:", CFQ_names[4]))
svy_glm <- svyglm(CFDAST~RIAGENDR+DMDMARTL+RIDRETH1+DMDEDUC2+Age_cat+BMI_cat+FIPR_cat+SMQ_cat+ALQ_cat+DMDBORN4,
                  design=svy_design)
reg_print(svy_glm)

print(paste("Response:", CFQ_names[5]))
svy_glm <- svyglm(CFDDS~RIAGENDR+DMDMARTL+RIDRETH1+DMDEDUC2+Age_cat+BMI_cat+FIPR_cat+SMQ_cat+ALQ_cat+DMDBORN4,
                  design=svy_design)
reg_print(svy_glm)
