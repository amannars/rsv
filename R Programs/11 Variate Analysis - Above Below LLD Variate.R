...
VAR_names <- PFAS_names[seq(from=4, to=nrow(PFAS_names), by=2),]
...
for (i_resp in 2:nrow(CFQ_names)){
  ...
  for (i in 1:length(VAR_names[,1])){
    ...
    num_col <- 1:ncol(data)
    comt_col <- num_col[col_match]
    conc_col <- comt_col - 1
    
    a_variate <- data[,comt_col]
    b_variate <- data[,conc_col]
    
    conc_above <- b_variate[substr(a_variate,1,1) == "A"] 
    t_test_a <- t.test(b_variate[substr(a_variate,1,1) == "A"])
    
    this_variate <- rep(NA, length(a_variate))
    this_variate[substr(a_variate,1,1) == "A"] <- "1 Above"
    this_variate[substr(a_variate,1,1) == "B"] <- "0 Below"
    
    table_a <- table(this_variate)
    
    N_below <- table_a[1]
    N_above <- table_a[2]
    mean_a <- t_test_a$estimate
    LCL_a <- t_test_a$conf.int[1]
    UCL_a <- t_test_a$conf.int[2]
    
    fmat_a <- "%20s     %5.0f  %5.0f   %5.3f   %5.3f   %5.3f"
    s1 <- sprintf(fmat_a,VAR_names[i], N_below, N_above, mean_a, LCL_a,UCL_a)
    print(s1)
    
    data$VARIATE <- as.factor(this_variate)
